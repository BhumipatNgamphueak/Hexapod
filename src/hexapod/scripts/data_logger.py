...
class DataLogger(Node):
    def __init__(self):
        ...
        # Base link data (track actual position via IMU integration)
        self.base_position = self.spawn_pos.copy()  # Start at spawn position
        self.base_velocity = np.zeros(3)
        self.prev_imu_time = None
        
        self.base_data = {
            'position': np.zeros(3),  # Relative to reference
            'velocity': np.zeros(3),
            'timestamp': 0.0
        }
        ...
        # Subscribe to IMU for base_link movement tracking
        self.create_subscription(
            Imu,
            '/imu/data',
            self.imu_callback,
    ...
    def imu_callback(self, msg):
        """Receive IMU data and integrate to estimate base position"""
        current_time = self.get_clock().now().nanoseconds / 1e9
        ...
        if self.prev_imu_time is not None:
            ...
            if dt > 0 and dt < 0.1:  # Sanity check on dt
                # Get linear acceleration (subtract gravity)
                accel = np.array([
                    msg.linear_acceleration.x,
                    msg.linear_acceleration.y,
                    msg.linear_acceleration.z + 9.81  # Remove gravity from Z
                ])
                
                # Integrate acceleration to velocity
                self.base_velocity += accel * dt
                
                # Integrate velocity to position
                self.base_position += self.base_velocity * dt
                
                # Apply damping to prevent drift (simple low-pass filter)
                self.base_velocity *= 0.99
    ...
    def log_data(self):
        ...
        # ===== LOG BASE_LINK DATA =====
        # Use IMU-integrated position (or spawn position if IMU not available)
        # Position relative to reference
        position_rel = self.base_position - self.ref_pos
        
        # Use integrated velocity from IMU
